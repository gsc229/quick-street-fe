...
import { About, VendorProducts, Bulletin, BannerUploader, Nav } from '../components/index';
						...
						<div className={banner.banner_text_btns}>
							<div className={banner.vendor_header_name}>
								{editingName ? ( // <<<<<<<<<< turnary ?
									...
									//<<<<<<< TURNARY input name or p tag
									<h1 onClick={() => setEditingName(true)} className={banner.business_name_text}>
										{vendorInfo.business_name}
									</h1>
								)}
							</div>
						</div>
>>>>>>> f92f28af146c9517320c0332813dee7676a5c181

						<div className={banner.vendor_banner_image_container}>
							{vendorInfo.vendor_banner !== `no-photo.jpg` ? (
								<CloudinaryContext cloudName="quickstlabs">
									<Image className={banner.vendor_banner_image} publicId={vendorInfo.vendor_banner}>
										<Transformation gravity="center" height="500" width="1800" crop="fill" />
									</Image>
								</CloudinaryContext>
							) : (
